...
def parse_job(cat_name, keyword, scrape_country, base_obj):
    ...
    print("PARSED DATA TO BE INSERTED INTO DATABASE",
          {"date_string": dt_string, "job title": job_title, "cat_name": cat_name, "job_link": job_link, "job_Description": job_desc, "location": location, "COMPANY": company, "keyword": keyword, "posted_date": posted_date, "Scape Country": scrape_country})
...
def get_job_desc():
    try:
        job_desc_element = driver.find_element(
            By.XPATH, "//div[@class='job-description-container']")
        print("JOB DESCRIPTION ELEMENT", job_desc_element)
        all_child_elements = job_desc_element.find_elements(By.XPATH, ".//*")
        job_desc = '\n'.join([d.text.strip() for d in all_child_elements])
...
if __name__ == "__main__":
    ...
    driver.get("https://us.jora.com/j?a=24h&l=United+States&q=python+developer")
    wait_job_cards = WebDriverWait(driver, 180).until(
        EC.presence_of_element_located(
    ...
    WebDriverWait(driver, 100).until(EC.presence_of_element_located(
        (By.XPATH, "//div[@class='job-description-container']")))
    parse_job("Web development", "python+developer", "United States", base_obj)
    time.sleep(60)
